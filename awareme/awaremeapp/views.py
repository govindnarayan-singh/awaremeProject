...
def orgRegister(request):
    ...
    if request.method == 'POST':
        ...
        if pass1 != pass2:
            ...
            messages.success(
                request, 'account created successfully with username as ' + username)
...
def createFeed(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, "article has been published successfully...")
            return redirect('listFeed')
...
def postComment(request, pk):
    if request.method == "POST":
        ...
        if parentSno == "":
            ...
            comment = FeedComment(
                comment=comment, writer=writer, post=post, parent=parent)
            comment.save()
            messages.success(
...
def search(request):
    ...
    if feed.count() == 0:
        messages.warning(
            request, " no results found please refine your query ")
...
def donating(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, "thank you for donating..")
            return redirect('listFeed')
